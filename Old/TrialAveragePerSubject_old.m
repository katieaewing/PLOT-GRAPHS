...
function [GRF_average, flexion_angles, flexion_moments, MF_average]=TrialAveragePerSubject(freq,subjectID,brace,task,trial,plotY)
%           
...
for x=task
    ...
    for y=trial
        ...
        %Find frame of end time (max knee flexion angle) in GRF data for non stop jump tasks
        if task<7
            maxIndGRF=1;
            while abs((outGRF.data(maxIndGRF,1) - MidTime(:,ii,jj)))> 0.0001
        ...
        else %for all other tasks, end time is time of max knee flexion (MidTime)
            
            maxIndMF=1;
            while abs((outMF.data(maxIndMF,1) - MidTime(:,ii,jj)))> 0.0001
                maxIndMF=maxIndMF+1;
            end
            maxIndMF;
